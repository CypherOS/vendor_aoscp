...
func init() {
	android.RegisterModuleType("aoscp_generator", GeneratorFactory)
...
func (g *Module) GenerateAndroidBuildActions(ctx android.ModuleContext) {
	...
	if depRoot == "" {
		...
		depRoot = aoscpExpandVariables(ctx, depRoot)
	}

	// Glob dep_files property
	for _, dep_file := range g.properties.Dep_files {
		dep_file = aoscpExpandVariables(ctx, dep_file)
	...
	cmd := aoscpExpandVariables(ctx, String(g.properties.Cmd))
